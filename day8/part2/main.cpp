...
int32_t MergeCircuits(std::vector<Circuit> &circuits)
{
...
int main()
{
    ...
    int32_t circuitsCount = points.size();
    ...
    for (size_t i = 0; i < pairs.size(); i++)
    {
        ...
        for (size_t j = 0; j < circuits.size(); j++)
        {
            ...
            if (aExists || bExists)
            {
                ...
                circuitFound = true;
                int32_t mergeCount = MergeCircuits(circuits);
    ...
    std::printf("a: %s, b: %s\n", lastBox.a.ToString().c_str(), lastBox.b.ToString().c_str());
    std::printf("x: %ld\n", lastBox.a.x * lastBox.b.x);
